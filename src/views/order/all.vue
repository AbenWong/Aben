    ...
    <div class="head-container">
      ...
      <el-drawer :visible.sync="showInfo" size="50%">
        <el-form ref="form" :model="orderInfo" :rules="rules" size="small" label-width="80px">
          ...
          <div class="info"
               style="display: flex;flex-direction: row;flex-wrap: wrap; padding-left:20px;padding-right:20px">
            <div style="display: flex;flex-direction: row;flex-wrap: wrap;">
              ...
              <el-form-item label="订单类型" style="width: 33%;color:blue">
            ...
            <el-form-item label="找零" style="width: 25%;">
              <span style="width: 100px">{{orderPayInfo.zhaoling}}</span>
            </el-form-item>
            ...
            <div class="title"></div>
            <div style="width: 100%;">
              <el-table :data="orderGoodsData" style="width: 100%; color:black" border
                        :header-cell-style="{background:'#ededed',color:'black'}">
                ...
                <el-table-column prop="goodsRate" label="扣率">
                  <template slot-scope="scope">
                    <span>{{scope.row.goodsRate}}%</span>
                  </template>
                </el-table-column>
                <el-table-column prop="goodsRate" label="扣率类型">
                  <template slot-scope="scope">
                    <span v-if="scope.row.goodsRateType==-1">未知扣率</span>
                    <span v-if="scope.row.goodsRateType==0">合同扣率</span>
                    <span v-if="scope.row.goodsRateType==1">商品扣率</span>
                    <span v-if="scope.row.goodsRateType==2">调整扣率</span>
                  </template>
                </el-table-column>
              </el-table>
            </div>
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;"
                :summary-method="getSummaries" @selection-change="crud.selectionChangeHandler">
      ...
      <div style="height: 40px;margin-top: 20px">
        <el-row>
          <el-col :span="12">
            <el-button type="success" :loading="crud.downloadLoading" @click="crud.doExport">导出</el-button>
          </el-col>
...
<script>
  ...
  export default {
    ...
    methods: {
      setQueryTime(time) {
        console.log(time);
        this.crud.query.orderTime = [];
        this.crud.query.orderTime.push(time[0] + " 00:00:00");
        if (time[0] === time[1]) {
          this.crud.query.orderTime.push(time[1] + " 23:59:59");
        } else {
          this.crud.query.orderTime.push(time[1] + " 00:00:00");
        }
      },
      // 获取数据前设置好接口地址
      [CRUD.HOOK.beforeRefresh]() {
        ...
        // 统计汇总订单数据
        // crudPosOrder.sumFee(this.crud.query).then(res => {
        //   this.sumObj = res;
        // })
        return true
      }, // 新增与编辑前做的操作
      ...
      getSummaries(param) {
        ...
        columns.forEach((column, index) => {
          ...
          sums[6] = this.sumObj.sumPayFee;
          sums[7] = this.sumObj.sumFee;
          sums[8] = this.sumObj.sumFreeFee;
        })
        return sums;
      },
      // 获取订单商品信息
